...
processIt = (file) ->
  ...
  process = new ffmpeg(video)
    .onCodecData (codecinfo) ->
      _log codecinfo
    .withSize(operations.dimensions)
    .withAspect(operations.aspect)
    .withFps(operations.fps)
    .withVideoCodec(operations.codec)
    #.addOption('-vtag', 'DIVX')
    #.usingPreset(operations.preset)
    .toFormat(operations.format)
