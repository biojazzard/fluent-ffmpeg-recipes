  ...
  */
  ...
  processIt = function(file) {
    ...
    return process = new ffmpeg(video).onCodecData(function(codecinfo) {
      return _log(codecinfo);
    }).withSize(operations.dimensions).withAspect(operations.aspect).withFps(operations.fps).withVideoCodec(operations.codec).toFormat(operations.format).saveToFile(render.file, function(stdout, stderr) {
