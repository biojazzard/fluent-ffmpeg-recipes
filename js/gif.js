...
function gen_256(width, height) {
  var gf = new omggif.GifWriter(buf, width, height);
  var palette = [ ];
  var indices = [ ];
  ...
  gf.addFrame(0, 0, width, 1, indices, {palette: palette});
  return buf.slice(0, gf.end());
}
...
*/
fs.writeFileSync('./gen_256.gif', gen_256(256, 256));
